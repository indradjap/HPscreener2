...
def quality_universe():
    ...
    universe_path=Path(__file__).with_name('idx_quality_200.csv')
    if not universe_path.exists():
        raise ValueError('Missing idx_quality_200.csv. Upload it in the same GitHub folder as app.py.')
    table=pd.read_csv(universe_path)
    if 'Ticker' not in table.columns:
        raise ValueError('idx_quality_200.csv must contain a Ticker column.')
    symbols=normalize_symbols(' '.join(table.Ticker.astype(str)))
    if len(symbols)!=200: raise ValueError(f'Bundled universe must contain 200 distinct tickers; found {len(symbols)}.')
    return symbols
